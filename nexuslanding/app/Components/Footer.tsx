...
export default function FooterNexus() {
  ...
  useEffect(() => {
    ...
    // Nexus-এর প্রতিটি লেটারকে আলাদা করে সিলেক্ট করা হচ্ছে
    const letters = nexusRef.current.querySelectorAll(".nexus-letter");

    gsap.fromTo(
      letters,
      {
        y: 100,         // শুরুতে লেটারগুলো ১০০ পিক্সেল নিচে থাকবে
        opacity: 0,      // এবং অদৃশ্য থাকবে
      },
      {
        y: 0,           // অ্যানিমেশন শেষে আসল জায়গায় ফিরবে
        opacity: 1,
        duration: 1,
        ease: "power4.out",
        stagger: 0.1,    // 🎯 মূল ট্রিক: একটি লেটার আসার ০.১ সেকেন্ড পর আরেকটি আসবে
        scrollTrigger: {
          trigger: nexusRef.current,
          start: "top 90%", // স্ক্রিনের নিচে আসামাত্র অ্যানিমেশন শুরু হবে
        },
      }
    );
  }, []);

  // "Nexus" শব্দটিকে প্রতিটি অক্ষরে ভাগ করা হয়েছে
  const word = "Nexus";

  return (
    <footer className="w-full pt-30 pb-5 rounded-t-[50px] overflow-hidden">
      <div className="container-custom mx-auto text-white overflow-hidden">
        ...
        <div 
          ref={nexusRef} 
          className="flex justify-center items-center overflow-hidden h-[150px] md:h-[250px] -mb-3"
        >
          {word.split("").map((char, index) => (
            <span 
              key={index} 
              className="nexus-letter inline-block text-[15vw] md:text-[18vw] font-black leading-none tracking-tighter"
            >
              {char}
            </span>
          ))}
